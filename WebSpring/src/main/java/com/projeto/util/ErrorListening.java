...
public class ErrorListening implements ErrorViewResolver {
	...
	public ModelAndView resolveErrorView(HttpServletRequest request, HttpStatus status, Map<String, Object> map) {
		...
		switch (status.value()) {
		case 404:
			model.addObject("error", "Página não econtrada!");
			model.addObject("message", "A url para a página '"+map.get("path") + "' não existe!");
			break;
		case 500:
			model.addObject("error", "Ocorreuy um erro interno no servidor!");
			model.addObject("message", "Ocorreu um erro inexperdo, tenta mais tarde!");
			break;	
		default:
